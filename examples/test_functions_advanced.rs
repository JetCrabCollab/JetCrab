...
fn main() {
    ...
    let result2 = engine.evaluate("let multiply = function(x, y) { return x * y; }; multiply(4, 6)");
    println!("   Result: {:?}", result2);
    
    println!("\n3. Testing function with parameters:");
    
    println!("\n   Step 3: Test function with parameters");
    let result3 = engine.evaluate("function greet(name) { return 'Hello ' + name; } greet('World')");
