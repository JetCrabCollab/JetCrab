    ...
    T: ControlFlowCore,
{
    if let Node::ForStatement(stmt) = node {
        ...
        if let Some(test) = &stmt.test {
            ...
            // Continue should jump to the update expression (or condition test)
            let continue_target = if let Some(_) = &stmt.update {
                // If there's an update, continue goes to the update expression
                // We need to find where the update expression starts
                // For now, we'll use the condition test position
                loop_start
            } else {
                // If no update, continue goes to the condition test
                loop_start
            };
    ...
    T: ControlFlowCore,
{
    if let Node::WhileStatement(stmt) = node {
        ...
        let jump_out_pos = if !is_true_literal {
            ...
            // For while(true), we don't generate a condition test
            None
        };
        ...
        // Patch all continue statements (Jump to 8888) in this loop
        // Continue should jump to the loop start (or condition test)
        let continue_target = if !is_true_literal {
            // For normal while loops, continue goes to the condition test
            loop_start
        } else {
            // For while(true), continue goes to the loop start
            loop_start
        };
    ...
    T: ControlFlowCore,
{
    if let Node::ForInStatement(stmt) = node {
        ...
        // TODO: Implement proper for...in semantics
    ...
    T: ControlFlowCore,
{
    if let Node::ForOfStatement(stmt) = node {
        // For now, implement a simplified version
        // TODO: Implement proper for...of semantics
        this.visit_node(&stmt.left);
        this.visit_node(&stmt.right);
        this.visit_node(&stmt.body);
    }
