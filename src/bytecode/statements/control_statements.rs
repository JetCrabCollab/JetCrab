    ...
    T: ControlFlowCore + LabelManager,
{
    if let Node::LabeledStatement(stmt) = node {
        ...
        this.add_label(
            format!("{}_start", label_name),
            CodeAddress::new(label_start),
        );
        ...
        this.add_label(format!("{}_end", label_name), CodeAddress::new(label_end));

        // Pop current label
        this.pop_current_label();

        // TODO: Handle break/continue with this label
        // We need to patch any break/continue statements that reference this label
    }
}
