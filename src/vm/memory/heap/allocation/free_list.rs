...
impl FreeListAllocator {
    ...
    fn find_best_fit(&self, size: usize) -> Option<FreeBlockId> {
        ...
        if let Some(Some(block_id)) = self.free_lists.get(&size) {
    ...
    fn remove_from_free_list(&mut self, block_id: FreeBlockId, size: usize) {
        if let Some(Some(head)) = self.free_lists.get(&size) {
            if *head == block_id {
                ...
                while let Some(block) = self.free_blocks.get(&current) {
                    if block.next == Some(block_id) {
                        ...
                        let next_next = if let Some(next_block) = self.free_blocks.get(&block_id) {
                            next_block.next
                        } else {
                            None
                        };
...
impl Allocator for FreeListAllocator {
    ...
    fn can_allocate(&self, size: MemorySize) -> bool {
        ...
        for &block_size in self.free_lists.keys() {
    ...
    fn fragmentation(&self) -> f64 {
        ...
        for _ in self.free_blocks.values() {
            free_blocks += 1;
        }
