...
impl IncrementalGc {
    ...
    fn perform_marking_increment(
        &mut self,
        _spaces: &mut HashMap<SpaceType, Box<dyn MemorySpace>>,
        target_time_ms: u64,
    ) -> Result<(), String> {
        let start_time = std::time::Instant::now();
        ...
        while start_time.elapsed().as_millis() < target_time_ms.into() {
    ...
    fn update_workload_estimates(&mut self) {
        ...
        let workload = &mut self.scheduling.workload_estimate;
    ...
    fn adjust_scheduling_parameters(&mut self) {
        ...
        // Ensure bounds
        self.scheduling.max_increment_time_ms = self.scheduling.max_increment_time_ms.clamp(1, 10);
    }
