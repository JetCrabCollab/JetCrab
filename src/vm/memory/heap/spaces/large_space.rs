...
impl LargeObjectSpace {
    ...
    fn split_region(&mut self, region_index: usize, requested_size: usize) {
        ...
        if remaining_size >= 1024 * 1024 { // Minimum 1MB for new region
    ...
    pub fn large_object_info(&self) -> LargeObjectSpaceInfo {
        ...
        for obj in self.large_objects.values() {
            *type_distribution.entry(obj.object_type.clone()).or_insert(0) += 1;
        }
        ...
        LargeObjectSpaceInfo {
            ...
            allocated_regions: self.memory_regions.iter().filter(|r| r.is_allocated).count(),
        }
    }
    ...
    pub fn should_compact(&self) -> bool {
        let allocated_regions = self.memory_regions.iter().filter(|r| r.is_allocated).count();
    ...
    pub fn compact(&mut self) -> CompactionStats {
        ...
        let mut allocated_regions: Vec<_> = self.memory_regions
            .iter()
        ...
        if current_address < self.total_size {
            ...
            if free_size >= 1024 * 1024 { // Minimum 1MB
                self.memory_regions.push(MemoryRegion {
                    id: self.next_region_id,
                    start_address: current_address,
                    size: free_size,
    ...
    fn calculate_fragmentation(&self) -> f64 {
        let total_free = self.memory_regions
            .iter()
            .filter(|r| !r.is_allocated)
            .map(|r| r.size)
            .sum::<usize>();
        
        let largest_free = self.memory_regions
            .iter()
            .filter(|r| !r.is_allocated)
            .map(|r| r.size)
            .max()
    ...
    pub fn efficiency(&self) -> f64 {
        let used = self.large_objects.values().map(|obj| obj.size).sum::<usize>();
...
impl MemorySpace for LargeObjectSpace {
    ...
    fn can_allocate(&self, size: MemorySize) -> bool {
        let size_bytes = size.as_usize();
        self.memory_regions.iter().any(|r| !r.is_allocated && r.size >= size_bytes)
    }
...
mod tests {
    ...
    fn test_large_object_space_collect() {
        ...
        assert_eq!(large_space.stats.object_count, before_objects - stats.objects_collected);
    }
