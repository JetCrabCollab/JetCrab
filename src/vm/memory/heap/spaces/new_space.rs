...
use crate::vm::memory::heap::allocation::{BumpAllocator, LayoutInfo};
use crate::vm::memory::heap::spaces::{GcStats, DefragmentationStats};
...
impl NewSpace {
    ...
    pub fn new(size: usize) -> Self {
        Self {
            active_space: BumpAllocator::new(size),
            inactive_space: BumpAllocator::new(size),
            space_size: size,
            stats: SpaceStats {
    ...
    pub fn memory_info(&self) -> NewSpaceInfo {
        ...
        NewSpaceInfo {
            ...
            total_used: active_info.used_size,
            total_free: active_info.free_size + inactive_info.free_size,
            usage_percentage: (active_info.used_size as f64 / (self.space_size * 2) as f64) * 100.0,
    ...
    pub fn collect(&mut self) -> GcStats {
        ...
        let before_usage = self.active_space.total_allocated().as_usize();
    ...
    pub fn remaining_space(&self) -> usize {
        self.active_space.remaining_space()
    }
    ...
    pub fn efficiency(&self) -> f64 {
        let active_used = self.active_space.total_allocated().as_usize();
...
impl MemorySpace for NewSpace {
    fn allocate(&mut self, size: MemorySize) -> Option<HeapHandleId> {
        if let Some(handle) = self.active_space.allocate(size) {
            ...
            // Update free space
            self.stats.free_size = self.active_space.total_free().as_usize() + 
                                  self.inactive_space.total_free().as_usize();
    ...
    fn total_free(&self) -> MemorySize {
        let active_free = self.active_space.total_free().as_usize();
        let inactive_free = self.inactive_space.total_free().as_usize();
...
mod tests {
    ...
    fn test_new_space_new() {
        ...
        assert_eq!(new_space.total_allocated().as_usize(), 0);
        assert_eq!(new_space.total_free().as_usize(), 2048); // 2 * 1024
    ...
    fn test_new_space_allocate() {
        ...
        assert_eq!(new_space.total_allocated().as_usize(), 64);
    ...
    fn test_new_space_reset() {
        ...
        assert_eq!(new_space.total_allocated().as_usize(), 64 + 128);
