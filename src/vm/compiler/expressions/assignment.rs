...
where
    T: AssignmentCore
        + crate::vm::compiler::scope::constants::ConstantCore
        + crate::vm::compiler::scope::local_vars::ScopeManager,
{
    fn generate_assignment_expression(&mut self, node: &Node) {
                        ...
                        Node::MemberExpression(member) => {
                            ...
                            match &*member.property {
                                Node::Identifier(name) if !member.computed => {
                                    ...
                                    let constant_id = <Self as crate::vm::compiler::scope::constants::ConstantManager>::add_constant(self, name.clone());
                                    self.instructions()
        ...
        if let Node::CallExpression(expr) = node {
            ...
            if let Node::MemberExpression(member) = &*expr.callee {
                    ...
                    (&*member.object, &*member.property)
                {
                    ...
                    // Check for console functions
                    if obj_name == "console" && (prop_name == "log" || prop_name == "error") {
                        let builtin_name = format!("console.{prop_name}");
                        // Push arguments first
                        for arg in &expr.arguments {
                            self.visit_node(arg);
                        }
                    ...
                    if obj_name == "JSON" && (prop_name == "stringify" || prop_name == "parse") {
                        let builtin_name = format!("JSON.{prop_name}");
                        // Push arguments first
                        for arg in &expr.arguments {
                            self.visit_node(arg);
                        }
            ...
            if let Node::Identifier(function_name) = &*expr.callee {
                // Function call by name
                let constant_id =
                    <Self as crate::vm::compiler::scope::constants::ConstantManager>::add_constant(
                        self,
                        function_name.clone(),
                    );
                self.instructions()
                    .push(Instruction::CallByName(constant_id));
            } else {
                // Other types of callees
