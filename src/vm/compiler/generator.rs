...
use crate::vm::compiler::literals::{
    ArrayCore, ArrayGenerator, FunctionLiteralCore, ObjectCore, ObjectGenerator, TemplateCore,
    TemplateGenerator,
};
...
impl BytecodeGenerator {
    ...
    fn visit_node(&mut self, node: &Node) {
        match node {
            ...
            Node::TemplateLiteral(_lit) => {
                // Generate bytecode for template literal
                <Self as TemplateGenerator>::generate_template_literal(self, node);
            }
...
impl crate::vm::compiler::statements::function::FunctionManagerCore for BytecodeGenerator {
    fn register_function(&mut self, function: crate::vm::function::Function) {
        self.function_manager.register_function(function);
    }
}
...
impl TemplateCore for BytecodeGenerator {
    fn instructions(&mut self) -> &mut Vec<Instruction> {
        &mut self.instructions
    }
    ...
    fn add_constant(&mut self, value: String) -> crate::vm::types::ConstantIndex {
        <Self as ConstantManager>::add_constant(self, value)
    }
}
