...
use crate::vm::runtime::context::Context;
...
impl Builtins {
    pub fn new() -> Self {
        ...
        builtins.register_console_functions();
        builtins.register_json_functions();
    ...
    fn register_console_functions(&mut self) {
        self.functions
            .insert("console.log".to_string(), Self::console_log);
        self.functions
            .insert("console.error".to_string(), Self::console_error);
    }

    fn register_json_functions(&mut self) {
        self.functions
            .insert("JSON.stringify".to_string(), Self::json_stringify);
        self.functions
            .insert("JSON.parse".to_string(), Self::json_parse);
    }
    ...
    // Console functions
    fn console_log(_context: &mut Context, args: &[Value]) -> Result<Value, String> {
        print!("console.log: ");
    ...
    fn console_error(_context: &mut Context, args: &[Value]) -> Result<Value, String> {
    ...
    // JSON functions
    fn json_stringify(_context: &mut Context, args: &[Value]) -> Result<Value, String> {
        if args.is_empty() {
    ...
    fn json_parse(_context: &mut Context, args: &[Value]) -> Result<Value, String> {
        if args.is_empty() {
        ...
        } else if let Ok(num) = json_str.parse::<f64>() {
            Ok(Value::Number(num))
        } else {
