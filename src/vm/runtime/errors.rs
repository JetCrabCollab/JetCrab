...
impl RuntimeError {
    ...
    pub fn type_error(operation: &str, expected_type: &str, actual_type: &str) -> Self {
        let message = format!(
            "TypeError: Cannot perform '{operation}' on type '{actual_type}', expected '{expected_type}'"
        );
    ...
    pub fn reference_error(identifier: &str, scope: &str) -> Self {
        let message = format!("ReferenceError: '{identifier}' is not defined in scope '{scope}'");
    ...
    pub fn range_error(value: &str, min: Option<&str>, max: Option<&str>) -> Self {
        let message = match (min, max) {
            (Some(min), Some(max)) => {
                format!("RangeError: Value '{value}' is out of range [{min}, {max}]")
            }
            (Some(min), None) => {
                format!("RangeError: Value '{value}' is below minimum '{min}'")
            }
            (None, Some(max)) => {
                format!("RangeError: Value '{value}' is above maximum '{max}'")
            }
            (None, None) => {
                format!("RangeError: Value '{value}' is out of valid range")
            }
        };
    ...
    pub fn syntax_error(
        ...
        let full_message = match (line, column, token) {
            (Some(line), Some(column), Some(token)) => {
                format!("SyntaxError: {message} at line {line}, column {column} (token: '{token}')")
            }
            (Some(line), Some(column), None) => {
                format!("SyntaxError: {message} at line {line}, column {column}")
            }
            (Some(line), None, None) => {
                format!("SyntaxError: {message} at line {line}")
            }
    ...
    pub fn error(message: &str, cause: Option<&str>) -> Self {
        let full_message = if let Some(cause) = cause {
            format!("Error: {message} (caused by: {cause})")
        } else {
...
pub mod helpers {
    ...
    pub fn invalid_array_length(length: usize) -> RuntimeError {
        RuntimeError::range_error(&length.to_string(), Some("0"), Some("2^32-1"))
    }
}
