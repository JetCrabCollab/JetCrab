...
//! ObjectHandler::new_object(&mut stack, &mut heap).unwrap();
//! ```
...
impl ObjectHandler {
    ...
    pub fn new_object<S, H>(stack: &mut S, heap: &mut H) -> Result<(), ExecutionError>
    where
        S: StackOperations,
        H: HeapOperations,
    {
    ...
    /// * `Ok(())` on success
    /// * `Err(ExecutionError::OutOfMemory)` if heap allocation fails
    pub fn new_class<S, H>(stack: &mut S, heap: &mut H) -> Result<(), ExecutionError>
    where
        S: StackOperations,
        H: HeapOperations,
    {
    ...
    pub fn set_property<S, H>(stack: &mut S, heap: &mut H) -> Result<(), ExecutionError>
    where
        S: StackOperations,
        H: HeapOperations,
    {
    ...
    /// * `Ok(())` on success
    /// * `Err(ExecutionError::StackUnderflow)` if insufficient operands
    pub fn set_property_assign<S, H>(stack: &mut S, heap: &mut H) -> Result<(), ExecutionError>
    where
        S: StackOperations,
        H: HeapOperations,
    {
    ...
    /// * `Ok(())` on success
    /// * `Err(ExecutionError::StackUnderflow)` if insufficient operands
    pub fn get_property<S, H>(stack: &mut S, heap: &mut H) -> Result<(), ExecutionError>
    where
        S: StackOperations,
        H: HeapOperations,
    {
    ...
    /// * `Ok(())` on success
    /// * `Err(ExecutionError::StackUnderflow)` if insufficient operands
    pub fn remove_object_property<S, H>(stack: &mut S, _heap: &mut H) -> Result<(), ExecutionError>
    where
        S: StackOperations,
        H: HeapOperations,
    {
        ...
        match (obj, key) {
            (Value::Object(_handle), Value::String(_key_str)) => {
        ...
        H: HeapOperations,
    {
        ...
        let result = match (&obj, method_name.as_str()) {
            (Value::String(_), "charAt") => {
                if let Some(Value::Number(_index)) = args.first() {
                    Value::String("a".to_string())
                } else {
                    Value::Undefined
                }
            }
            (Value::Array(_), "push") => Value::Number(args.len() as f64),
            (Value::Array(_), "pop") => Value::Undefined,
            _ => Value::Undefined,
        };
